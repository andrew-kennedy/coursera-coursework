...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] =
    if (singleton(trees) || trees == Nil) trees
      ...
      makeCodeTree(trees.head, trees.tail.head) :: trees.drop(2) sortBy weight
  ...
  /**
    ...
    */
  def until[T](pred: T => Boolean, mapFunc: T => T)(trees: T): T =
    if (pred(trees)) trees
    else until(pred, mapFunc)(mapFunc(trees))
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def traverse(remaining: CodeTree, remainingBits: List[Bit]): List[Char] = {
      remaining match {
        case Fork(l, _, _, _) if remainingBits.head == 0 =>
          traverse(l, remainingBits.tail)
        case Fork(_, r, _, _)                    => traverse(r, remainingBits.tail)
        case Leaf(c, _) if remainingBits.isEmpty => List(c)
        case Leaf(c, _)                          => c :: traverse(tree, remainingBits)
      }
    }
    traverse(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def retrieve(tree: CodeTree)(c: Char): List[Bit] = tree match {
      case Leaf(_, _)                               => List()
      case Fork(l, _, _, _) if chars(l).contains(c) => 0 :: retrieve(l)(c)
      case Fork(_, r, _, _)                         => 1 :: retrieve(r)(c)
    }
    text flatMap retrieve(tree)
  }
