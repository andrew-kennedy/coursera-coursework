...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    val bothHalves = right.filterAcc(p, filteredLeft)
    if (p(elem)) bothHalves.incl(elem) else bothHalves
  }

  override def mostRetweeted: Tweet = {
    val mostLR = List(left, right) collect {
      // collect all the mostRetweeted tweets from the left and right subsets (ignoring empty sets) into a list
      case t: NonEmpty => t.mostRetweeted
    }
    elem :: mostLR maxBy (_.retweets)
  }
...
object GoogleVsApple {
  ...
  // we check if the text of each tweet co
